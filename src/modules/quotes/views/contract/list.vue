  ...
  <div class="app-container contract-list">
    ...
    <el-table ref="multipleTable" tooltip-effect="dark" :data="list" border fit highlight-current-row>
      <el-table-column label="合同编号" width="180" align="center">
        <template scope="scope"><span>{{scope.row.orderNum}}</span></template>
      </el-table-column>
      <el-table-column label="订单名称" align="center" show-overflow-tooltip>
        <template scope="scope"> {{scope.row.orderName}}        </template>
      </el-table-column>
      <el-table-column label="采购日期" width="120" align="center">
        <template scope="scope"> {{scope.row.purchaseData}}        </template>
      </el-table-column>
      ...
      <el-table-column label="交货日期" align="center" width="120">
        <template scope="scope"> {{scope.row.deliveryData}}        </template>
      </el-table-column>
      ...
      <el-table-column label="订单状态" width="100" align="center">
        <template scope="scope"><span v-if="scope.row.orderStatus === 0">待确认</span> <span
          v-if="scope.row.orderStatus === 1">首付待付款</span> <span v-if="scope.row.orderStatus === 2">交付中</span> <span
          v-if="scope.row.orderStatus === 3">尾款待付款</span> <span v-if="scope.row.orderStatus === 4">已完成</span></template>
      </el-table-column>
      <el-table-column label="操作" width="300" align="center">
        <template scope="scope">
          <el-button size="small" v-if="scope.row.orderStatus === 0 " @click="dialogShow('是否确定合同?')">确认合同</el-button>
          <el-button @click="hrefHandle(scope.row.orderStatus)" size="small" style="margin: 20px"> 查看详情
          </el-button>
...
export default {
  data() {
    return {list: null, activeName: 'first'}
  }, filters: {}, created() {
    this.fetchData()
  }, methods: {
    handleClick(tab, event) {
    }, dialogShow(text) {
      this.$confirm(text, '温馨提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
      }).catch(() => {
      });
    }, hrefHandle(index) {
      this.$router.push({path: '/contract/detail', query: {id: index}})
    }, fetchData() {
      this.list = Api.contractList;
    }
  }
}</script>
<style rel="stylesheet/scss" lang="scss">  .contract-list {
  .img-size {
    width: 100px;
    height: 100px;
    object-fit: contain;
